...
namespace Gamora_Indumentaria.Data
{
    ...
    public static class DatabaseManager
    {
        ...
        /// <summary>
        /// Inserta talles faltantes para algunas categorías (ej. Pantalones 36..55) si no existen.
        /// Esto asegura que nuevas instalaciones tengan un conjunto básico de talles.
        /// </summary>
            ...
            try
            {
                using (SqlConnection connection = GetConnection())
                {
                    ...
                    string createTables = @"
                        ...
                        END
                        -- Agregar columna FechaModificacion si no existe (usada por actualizaciones)
                        IF NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.COLUMNS 
                                     WHERE TABLE_NAME = 'Inventario' AND COLUMN_NAME = 'FechaModificacion')
                        BEGIN
                            ALTER TABLE Inventario ADD FechaModificacion DATETIME NULL;
                        END
        ...
        public static int ProcesarVenta(List<ItemCarrito> carrito, string metodoPago, decimal totalVenta, bool esRegalo = false)
        {
            var detalles = carrito.Select(item => new ItemVenta
            {
                ...
                // item.Subtotal ya contiene el subtotal neto (descuento aplicado), no volver a aplicarlo
